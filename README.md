...
> - Low overhead: A database uses 2048(x2) bytes, plus 24(x2) bytes per record, plus the space for keys and data.
> - No random limits: cdb can handle any database up to 4 (x2**16) gigabytes. There are no other restrictions; records don't even have to fit into memory. Databases are stored in a machine-independent format.
